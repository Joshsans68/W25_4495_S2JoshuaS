...
document.addEventListener("DOMContentLoaded", () => {
    ...
    async function loadTeamDashboard(teamId, teamName) {
        ...
        fetchLiveScores();  // Fetch live scores across all games
        ...
        try {
            ...
            if (data.response.length > 0) {
                ...
                data.response.forEach(match => {
                    ...
                    el.textContent = `${match.teams.home.name} ${match.goals.home} - ${match.goals.away} ${match.teams.away.name} (${match.fixture.status.long})`;
    ...
    async function fetchMatchHistory(teamId) {
        ...
        try {
            ...
            // If 2023 data is unavailable, fallback to 2022
            if (data.response.length === 0) {
            ...
            data.response.forEach(function(match) {
                const el = document.createElement("p");
        ...
        try {
            ...
            if (teamRow) {
                container.innerHTML = `
                    ...
                    <p>Position: #${teamRow.rank} | Points: ${teamRow.points}</p>
                    <p>Wins: ${teamRow.all.win}, Draws: ${teamRow.all.draw}, Losses: ${teamRow.all.lose}</p>
                `;
            } else {
